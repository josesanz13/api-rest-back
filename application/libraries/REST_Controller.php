...
abstract class REST_Controller extends CI_Controller {
    ...
    protected $http_status_codes = [
        ...
        self::HTTP_INVALID_TOKEN=> 'INVALID TOKEN'
    ];
    ...
    public function validate_token_api( $data,$token="" ) {
        $response = '';
        $user_id = $data['id'];
        $current_date = date('Y-m-d');
        $validate_token = $token != "" ? " AND access_token = {$this->db->escape($token)} " : "";

        $sql = "
            SELECT id,access_token 
            FROM user_token_authorization 
            WHERE user_id = {$user_id} AND update_token = {$this->db->escape($current_date)} {$validate_token}
        ";

        if ( $query = $this->db->query( $sql ) ) {
            if ( $query->num_rows() > 0 ) {
                $query = $query->row_array();
                $response = $query['access_token'];
            }
        }
    ...
    public function renew_token_api( $data,$token ) {
        $response = false;
        $user_id = $data['id'];
        $current_date = date('Y-m-d');

        $sql = "SELECT id FROM user_token_authorization WHERE user_id = {$user_id}";
        ...
        if ( $query = $this->db->query( $sql ) ) {
            if ( $query->num_rows() > 0 ) {
                ...
                $sql = " 
                    INSERT INTO user_token_authorization (
                        user_id,
                        access_token,
                        update_token
                    )
    ...
    public function validate_request( $headers ){
        // Validate headers
        if ( !validate_headers_api( $headers,true ) ) {
            return "The ' Auhtorization and Content-Type ' headers has not been found [ Content-Type : application/json - Authorization : JWT ]";
        }
